...
def load_indexBcs(tsv: Path) -> Dict[str, str]:
    """
...
def assign_index(libraries: List[Library], libDef: Dict, reads: Dict[str, ReadInfo]) -> IndexInfo:
    ...
    index1Used = ('I1' in reads) and any(l.index1 for l in libraries)
    index2Used = ('I2' in reads) and any(l.index2 for l in libraries)
...
def main(samplesCsv: Path, libJson: Path, runInfo: Path, splitFastq: bool, settings):
    ...
    if splitFastq and not libDef["split_on"]:
        raise Exception("If using splitFastq please provide either index1 or index2"
...
if __name__ == '__main__':
    ...
    main(args.samples, args.libDef, args.runinfo, args.splitFastq, SETTINGS)